...
Run the installer.  When you first start, the pocketnetcore desktop utility will ask for the location of the blockchain data directory. Default for Windows %APPDATA%/Pocketcoin, for linux ~/.pocketcoin.
Download the checkpoints file before starting the pocketcoin client for the first time.  Below are the steps to download it from a cmd window:
```
cd %APPDATA%
mkdir Pocketcoin
cd Pocketcoin
